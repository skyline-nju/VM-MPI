...
void run_quenched(int gl_par_num, const Vec_2<double>& gl_l, double eta, double eps,
  ...
  snprintf(logfile, 100, "data%s%d.%d.%d.%llu_%03d.log",
    exporter::delimiter.c_str(), int(gl_l.x), int(eta * 1000), int(eps * 1000), seed, seed2);
  snprintf(phifile, 100, "data%sp%d.%d.%d.%llu.%03d.dat",
    exporter::delimiter.c_str(), int(gl_l.x), int(eta * 1000), int(eps * 1000), seed, seed2);
  snprintf(snapfile, 100, "data/snap%ss%d.%d.%d.%03d.%llu",
    exporter::delimiter.c_str(), int(gl_l.x), int(eta * 1000), int(eps * 1000), seed, seed2);
  snprintf(field_f1, 100, "data/RT_field_%d_%.3f_%.3f_%d_%llu_%03d.bin", int(gl_l.x), eta, eps, field_dt1, seed, seed2);
  snprintf(time_ave_f1, 100, "data/RT_ave_%d_%.3f_%.3f_%d_%llu_%03d.bin", int(gl_l.x), eta, eps, ave_dt1, seed, seed2);
#elif defined RANDOM_FIELD
  ...
  exporter::FeildExporter field_ex1(field_f1, field_t_beg1, n_step * 2, field_dt1, grid, dm, 2);
...
void run_RO(int gl_par_num, const Vec_2<double>& gl_l,
  ...
  char basename[100];
  if (gl_l.x == gl_l.y) {
    snprintf(basename, 100, "%g_%.3f_%.5f_%g_%llu_%03d", gl_l.x, eta, rho_s, eps, seed, seed2);
  } else {
    snprintf(basename, 100, "%g_%g_%.3f_%.5f_%g_%llu_%03d", gl_l.x, gl_l.y, eta, rho_s, eps, seed, seed2);
  }
  snprintf(logfile, 100, "data/RO_%s.log", basename);
  snprintf(phifile, 100, "data/RO_%s.dat", basename);
  snprintf(snapfile, 100, "data/snap/RO_%s", basename);
  snprintf(field_f1, 100, "data/RO_field_%s_%d.bin", basename, field_dt1);
  snprintf(time_ave_f1, 100, "data/RO_ave_%s_%d.bin", basename, ave_dt1);
